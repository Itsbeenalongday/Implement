...
int main() {
	...
	auto itr = set_intersection(don_listen.begin(), don_listen.end(), don_see.begin(), don_see.end(), result.begin());
	result.erase(itr, result.end()); // 반환값이, 삽입이 끝난후의 끝 지점 iterator
	cout << result.size()-1;
	for_each(result.begin(), result.end(), [&result](string str) { // python의 map과 동일
		cout << str << '\n';
	});
	return 0;
}